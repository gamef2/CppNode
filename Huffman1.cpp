...
public:
	...
	bool operator== (const DataRec& r) const { // used by find();
		return symbol == r.symbol && runLen == r.runLen;
	}
	bool operator< (const DataRec& r) const {  // used by sort();
		return freq < r.freq;
	}
};
...
//	unsigned int dataSize;
	...
	unsigned long charCnt, dataSize;
	ofstream fOut;
	...
	void write4ByteNum(unsigned long& num, ofstream& fOut);
...
void HuffmanCoding::outputFrequencies(ifstream& fIn) {
	unsigned long temp4;
	char ch;// = data.size();
	unsigned long temp2 = data.size();

	//temp2 >>= bits;
	//fOut.put(char(temp2)).put(ch);
	write4ByteNum(temp2, fOut);
	cout << fIn.tellg() << endl;
	fIn.clear();
	cout << fIn.tellg() << endl;
...
void HuffmanCoding::write4ByteNum(unsigned long& num, ofstream& fOut) {
	int bit_v = 32;
	for (int i = 0; i < 4; i++) {
		bit_v -= 8;
...
void HuffmanCoding::encode(ifstream& fIn) {
	...
	for (fIn.get(ch); !fIn.eof(); ) {
		...
		for (p = chars[(unsigned char)ch];
			...
			;
		if (p == 0)
			error((char*)"A problem in encode()");
	...
	if (packCnt != 0) {
		pack = pack<<(maxPack - packCnt); // transfer leftover codewords and some 0's
		output(pack);
	}
}
